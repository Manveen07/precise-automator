...
def update_heyreach_sequence_now(campaign_id: str) -> dict:
    """Re-push sequence to existing HeyReach campaign. Used by Re-create button."""
...
async def _update_sequence_async(campaign_id: str) -> dict:
    """Push a new sequence to an existing HeyReach campaign (used by Re-create)."""
    ...
    if not hr_campaign_id:
        # No existing campaign — fall back to full create
    ...
    workspace = get_workspace_config(doc.get("smartlead_workspace", ""))
    if not workspace or not workspace.get("heyreach_api_key"):
        summary["errors"].append("HeyReach API key not configured")
        return summary
